...
export function validateConnectionInput(data: Partial<CreateConnectionRequest>): { isValid: boolean; error?: string } {
  ...
  if (data.bandwidth !== undefined) {
    if (typeof data.bandwidth !== 'number' ||
      data.bandwidth < CONNECTION_VALIDATION.BANDWIDTH_MIN ||
      data.bandwidth > CONNECTION_VALIDATION.BANDWIDTH_MAX) {
...
export function getConnectionDisplayName(connection: Connection): string {
  const portInfo = connection.portFrom && connection.portTo
    ? ` (${connection.portFrom} → ${connection.portTo})`
    : ''
...
export function isConnectionBidirectional(connection: Connection): boolean {
  return connection.category === ConnectionCategory.NETWORK ||
    connection.category === ConnectionCategory.DATA ||
    connection.type === ConnectionType.LOGICAL
}
